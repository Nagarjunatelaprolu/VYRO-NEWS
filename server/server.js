...
app.post("/api/news",auth,(req,res)=>{
  ...
  const r=db.prepare(`INSERT INTO news(title,category,content,image_url,breaking,featured,status,created_at,updated_at)
    VALUES(?,?,?,?,?,?,?,?,?)`).run(n.title,n.category,n.content,n.image_url||"",n.breaking?1:0,n.featured?1:0,n.status==="Published"?"Published":"Draft",now,now);
...
app.listen(PORT,()=>console.log(`VYRO NEWS API: http://localhost:${PORT}`));
